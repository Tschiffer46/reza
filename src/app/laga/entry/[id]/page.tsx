...
import { ENTRY_SHARES_INCLUDE, canSeeEntry, entryFamilies, normalizeVisibility } from '@/lib/entry-access'
...
export default async function EntryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canSeeEntry(entry ? { ...entry, shares: entry!.shares.map((sh) => ({ familyId: sh.family.id })) } : null, userId, ids)) {
    notFound()
  }
  ...
  // Skapare får alltid radera egna; annars krävs betald + gemenskaps-admin, eller global
  // admin. Ett privat recept är skaparens ensak — ingen annan väg in.
  const canDelete =
    entry!.creatorId === userId ||
    (normalizeVisibility(entry!.visibility) !== 'private' &&
